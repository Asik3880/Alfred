...
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, SUPPORT_CHAT_ID, CUSTOM_FILE_CAPTION, MSG_ALRT, PICS, AUTH_GROUPS, P_TTI_SHOW_OFF, GRP_LNK, CHNL_LNK, SPRT_CHNL, NOR_IMG, LOG_CHANNEL, SPELL_IMG, MAX_B_TN, IMDB, \
    SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, NO_RESULTS_MSG, TUTORIAL, HOW_TO_VERIFY, REQST_CHANNEL, IS_TUTORIAL, LANGUAGES, SEASONS, QUALITY, SUPPORT_CHAT, PREMIUM_USER
    ...
    if settings['button']:
        ...
        btn.insert(0, 
            [
                ...
                InlineKeyboardButton(f'ʀᴜʟᴇꜱ', 'select')
            ]
        )
        # btn.insert(0, [
...
async def qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    for i in range(0, len(QUALITY)-1, 2):
        btn.append([
            InlineKeyboardButton(
                text=QUALITY[i].upper(),
                callback_data=f"fq#{QUALITY[i].lower()}#{key}"
            ),
            InlineKeyboardButton(
                text=QUALITY[i+1].upper(),
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="👇 sᴇʟᴇᴄᴛ ʏᴏᴜʀ ǫᴜᴀʟɪᴛɪᴇs 👇", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
...
async def filter_quality_cb_handler(client: Client, query: CallbackQuery):
    ...
    search = FRESH.get(key)
    search = search.replace("_", " ")
    baal = quality in search
    if baal:
        search = search.replace(quality, "")
    else:
        search = search
    ...
    if settings["button"]:
        ...
        for file in files:
            btn.append([
                InlineKeyboardButton(text=f"[{get_size(file.file_size)}] {' '.join(filter(lambda x: not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}", url=f'https://telegram.dog/{temp.U_NAME}?start=short_{chat_id}_{file.file_id}')
            ])
        btn.insert(0, 
            [
                InlineKeyboardButton("ʟᴀɴɢᴜᴀɢᴇꜱ", callback_data=f"languages#{key}"),
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("🌿 Mᴏᴠɪᴇ Sᴇᴀʀᴄʜ Gʀᴏᴜᴘ 🥀", url="https://t.me/+ipp-HIbjDIo4ZTll")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        buttons = [[
            InlineKeyboardButton('∙ 🇬‌🇷‌🇴‌🇺‌🇵‌ 🇲‌🇪‌🇳‌🇺‌ ∙', callback_data='misc')
        ], [
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.MISC_TXT.format(query.from_user.mention, temp.U_NAME, temp.B_NAME),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
...
async def advantage_spell_chok(client, msg):
    ...
    try:
        ...
        InlineKeyboardButton("🔍 ᴄʟɪᴄᴋ ᴛᴏ ᴄʜᴇᴄᴋ ꜱᴘᴇʟʟɪɴɢ ✅", url=f"https://www.google.com/search?q={reqst_gle}+movie")
        ],[
